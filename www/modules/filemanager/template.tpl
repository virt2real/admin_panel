<script>
	...
	function SaveFile(file, text){		$("#savestatus").html('<img src="/imgs/loader.gif">');
		...
		$.post("modules/{module_name}/save.php?rnd=" + Math.random(), {file: file, text: text}, function(response, status, xhr) {			if (status == "success") {
				$("#savestatus").html("Файл успешно сохранён");
				$("#savebutton").prop("disabled", true);
			}
			if (savestatus == "error") {
				$("#status").html(":-(");
...
<div id="accordion" style="margin:0; padding:0;">
	...
	<h3><a href="#">Файл-менеджер и редактор файлов</a></h3>
	<div>
		<div id="status" style="height: 26px; text-align: right; width: 100%;"></div>

		<div id="editfileblock" style="display: none; height: auto;">
			<p><a href="#" onclick='$("#editfileblock").css("display", "none"); $("#filelist").css("display", "block"); return false;'>[ Назад ]</a></p>
			<div id="fileedit" style="height: auto;"></div>
			<p></p>
			<div>
				<input type="button" value="Сохранить" id="savebutton" disabled="disabled" onclick='SaveFile($("#filename").html(), $("#filecontent").val()); return false;'>
				<input type="button" value="Вернуться" onclick='$("#editfileblock").css("display", "none"); $("#filelist").css("display", "block"); return false;'>
